...
export class LmsHttpClient {
  ...
  /** 상대 경로를 절대 URL 로 만들고, LMS 호스트가 아니면 거부한다. */
  resolve(pathOrUrl: string): string {
    const url = new URL(pathOrUrl, `${this.opts.baseUrl}/`);
    if (url.host !== this.opts.lmsHost) {
      throw new LmsError('UNSUPPORTED', `LMS 호스트(${this.opts.lmsHost}) 외부(${url.host})로는 요청하지 않습니다`, { retryable: false });
    }
    return url.toString();
  }
  ...
  async request<T = string>(pathOrUrl: string, options: RequestOptions = {}): Promise<HttpResponse<T>> {
    ...
    while (attempt <= maxRetries) {
      ...
      try {
        ...
        const backoff = Math.min(4000, 400 * 2 ** attempt) + Math.floor(Math.random() * 200);
  ...
  private async once<T>(startUrl: string, method: 'GET' | 'POST', options: RequestOptions): Promise<HttpResponse<T>> {
    ...
    for (;;) {
      ...
      if (res.status >= 300 && res.status < 400) {
        ...
        if (next.host !== this.opts.lmsHost) {
          // 외부 호스트로는 따라가지 않고, 쿠키도 보내지 않는다.
      ...
      if (kind) {
        throw new LmsError(kind, `HTTP ${res.status}`, { retryable: kind === 'SERVER' || kind === 'TIMEOUT' || kind === 'RATE_LIMITED' });
      }
