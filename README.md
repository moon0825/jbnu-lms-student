...
실제 수강 과목이 보이는 LMS 화면에 도착하면 창을 닫지 않고 잠시 기다립니다. 도구가 전용 Chrome을 세션 보존 종료한 뒤 LMS 쿠키만 읽어
HTTP로 검증하고, 원래 질문을 이어서 처리합니다. 두 번째 `패스키 인증 로그인` 탭의 비밀번호 없는 단독 로그인과는
...
| 모드 | 설명 | 설정 |
|---|---|---|
| `plain` (기본) | 자동화가 전혀 없는 일반 브라우저. LMS 홈을 감지하면 전용 Chrome만 종료해 DPAPI 세션 저장 후 원문 보기용 최소 프로필로 정리 | `JBNU_LMS_LOGIN_MODE=plain` |
